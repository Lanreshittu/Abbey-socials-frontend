...
export default function UserRelationships({ friendsData, followingData, followersData }: any) {
  ...
  return (
    <div>
      {/* Tab Navigation */}
      <div className="flex justify-around bg-gray-800 p-4 mb-4">
        <div
      ...
      <ul
        role="list"
        className="divide-y divide-gray-700 overflow-hidden bg-gray-900 shadow-lg ring-1 ring-gray-700 sm:rounded-xl"
      >
        {getActiveData().map((person: any) => (
          <li
            key={person.email}
            className="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-800 sm:px-6"
          >
            <div className="flex min-w-0 gap-x-4">
              {/* <img alt="" src={person.imageUrl} className="h-12 w-12 flex-none rounded-full bg-gray-700" /> */}
            ...
            <div className="flex shrink-0 items-center gap-x-4">
              <div className="hidden sm:flex sm:flex-col sm:items-end">
                <p className="text-sm leading-6 text-gray-100">{person.role}</p>
                {person.lastSeen ? (
